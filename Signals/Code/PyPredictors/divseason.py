...
def divseason():
    ...
    Constructs the DivSeason predictor signal for predicted dividend month.
    """
    ...
    try:
        ...
        master_vars = ['permno', 'time_avail_m']
        ...
        # SIGNAL CONSTRUCTION
        logger.info("Calculating DivSeason signal...")
        ...
        # Create divpaid flag (equivalent to Stata's "gen divpaid = divamt > 0")
        merged_data['divpaid'] = (merged_data['divamt'] > 0).astype(int)
        
        # Drop monthly dividends (equivalent to Stata's "drop if cd3 == 2")
        merged_data = merged_data[merged_data['cd3'] != 2]
        
        # Keep only cd3 < 6 (equivalent to Stata's "keep if cd3 < 6")
        merged_data = merged_data[merged_data['cd3'] < 6]
        
        # Calculate 12-month rolling sum of divpaid (equivalent to Stata's "asrol divpaid, window(time_avail_m 12) stat(sum) gen(div12)")
        merged_data['div12'] = merged_data.groupby('permno')['divpaid'].rolling(window=12, min_periods=1).sum().reset_index(0, drop=True)
        
        # Initialize DivSeason to 0 if div12 > 0 (equivalent to Stata's "gen DivSeason = 0 if div12 > 0")
        merged_data['DivSeason'] = np.where(merged_data['div12'] > 0, 0, np.nan)
        
        # Calculate lags for dividend prediction
        merged_data['divpaid_lag2'] = merged_data.groupby('permno')['divpaid'].shift(2)
        merged_data['divpaid_lag5'] = merged_data.groupby('permno')['divpaid'].shift(5)
        merged_data['divpaid_lag8'] = merged_data.groupby('permno')['divpaid'].shift(8)
        merged_data['divpaid_lag11'] = merged_data.groupby('permno')['divpaid'].shift(11)
        
        # Create temp flags for different dividend frequencies (equivalent to Stata's temp3, temp4, temp5 logic)
        # Quarterly, unknown, and missing frequency (assumed quarterly)
        merged_data['temp3'] = ((merged_data['cd3'].isin([3, 0, 1])) & 
                               ((merged_data['divpaid_lag2'] == 1) | 
                                (merged_data['divpaid_lag5'] == 1) | 
                                (merged_data['divpaid_lag8'] == 1) | 
                                (merged_data['divpaid_lag11'] == 1))).astype(int)
        
        # Semi-annual
        merged_data['temp4'] = ((merged_data['cd3'] == 4) & 
                               ((merged_data['divpaid_lag5'] == 1) | 
                                (merged_data['divpaid_lag11'] == 1))).astype(int)
        
        # Annual
        merged_data['temp5'] = ((merged_data['cd3'] == 5) & 
                               (merged_data['divpaid_lag11'] == 1)).astype(int)
        
        # Set DivSeason to 1 if any temp flag is true (equivalent to Stata's "replace DivSeason = 1 if temp3 | temp4 | temp5")
        merged_data.loc[(merged_data['temp3'] == 1) | (merged_data['temp4'] == 1) | (merged_data['temp5'] == 1), 'DivSeason'] = 1
        
        logger.info("Successfully calculated DivSeason signal")
        
        # SAVE RESULTS
        logger.info("Saving DivSeason predictor signal...")
        ...
        output_data = merged_data[['permno', 'time_avail_m', 'DivSeason']].copy()
        
        # Remove missing values
        output_data = output_data.dropna(subset=['DivSeason'])
        ...
        # Save CSV file
        csv_output_path = predictors_dir / "DivSeason.csv"
        csv_data = output_data[['permno', 'yyyymm', 'DivSeason']].copy()
        csv_data.to_csv(csv_output_path, index=False)
        logger.info(f"Saved DivSeason predictor to: {csv_output_path}")
        
        logger.info("Successfully constructed DivSeason predictor signal")
        return True
        
    except Exception as e:
        logger.error(f"Failed to construct DivSeason predictor: {e}")
        return False
